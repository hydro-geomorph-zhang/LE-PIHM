...
void
is_sm_et(realtype t, realtype stepsize, Model_Data DS, N_Vector VY)
{
	int             i;
	realtype        totEvap;
	...
	realtype		snow_last, is_last,balance;
    ...
    for (i = 0; i < MD->NumEle; i++) {
		...
		if (DS->Keyword_LE_PIHM_HYDRO == 1)
		{
			...
			{
				if (DS->Keyword_FULLY_COUPLE == 1)
				{
					P = 101.325 * pow(10, 3) * pow((293 - 0.0065 * NV_Ith_S(VY, i + 3*MD->NumEle)) / 293, 5.26);
				}
				else
				{
