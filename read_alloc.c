...
//  	printf("done.\n");
  	...
  	/*========== open *.LE_att file ==========*/
//  	printf("\n  3) reading %s.att  ... ", filename);
  	fn[13] = (char *)malloc((strlen(filename)+1024)*sizeof(char));
  	strcpy(fn[13], filename);
  	LE_att_file = fopen(strcat(fn[13], ".LE_att"), "r");
	free(fn[13]);
  	if(LE_att_file == NULL)
  		{
    		printf("\n  Fatal Error: %s.att is in use or does not exist!\n", filename);
  	...
  	/* start reading LE_att_file */ 
  	for (i=0; i<DS->NumEle; i++)
  		{
    		fscanf(LE_att_file, "%d", &(tempindex));
    		fscanf(LE_att_file, "%d %d", &(DS->Ele[i].LE_soil), &(DS->Ele[i].LE_geol));
 		}
...
//  	printf("\n  8) reading %s.ibc  ... ", filename);  
 ...
 // 	printf("done.\n");
  ...
  //	printf("\n  9) reading %s.para ... ", filename); 
	...
	fscanf(para_file, "%s %d", tempchar, &(DS->Keyword_FULLY_COUPLE));
	fscanf(para_file, "%s %d", tempchar, &(DS->Keyword_LE_PIHM));
	fscanf(para_file, "%s %d", tempchar, &(DS->Keyword_BEDROCK));
	fscanf(para_file, "%s %d", tempchar, &(DS->Keyword_LE_PIHM_SED));
	fscanf(para_file, "%s %d", tempchar, &(DS->Keyword_LE_PIHM_HYDRO));
	fscanf(para_file, "%s %d", tempchar, &(DS->Keyword_LE_PIHM_SUBHYDRO));
	fscanf(para_file, "%s %d", tempchar, &(DS->Keyword_REALTIME));
	fscanf(para_file, "%s %d", tempchar, &(DS->Keyword_MEAN));
	fscanf(para_file, "%s %d", tempchar, &(DS->Keyword_BINARY));
	fscanf(para_file, "%s %lf", tempchar, &(DS->Constant_precip));
    ...
    fscanf(para_file, "%s %d %s %d %s %d %s %d %s %d", tempchar, &(CS->RechInt), tempchar, &(CS->IsDInt), tempchar, &(CS->usDInt), tempchar, &(CS->etInt), tempchar, &(CS->computtimeDInt));
  	...
  	fscanf(para_file, "%s %d %s %d %s %d", tempchar, &DS->UnsatMode, tempchar, &DS->SurfMode, tempchar, &DS->SedMode);
